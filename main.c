...
SEXP assign_items(
    ...
    if (!isReal(weightsSEXP)) error("weights must be type double.");
    if (!isInteger(durationsSEXP)) error("durations must be type integer.");
    if (!isReal(strengthSEXP)) error("strength must be type double.");

    int n = LENGTH(weightsSEXP);
    int p = LENGTH(strengthSEXP);

    if (LENGTH(durationsSEXP) != n) error("weights and durations must be the same lenght");
    if (p <= 0) error("There must be at least one person");
    ...
    for(int i = 0; i < p; i++) {
        if(weights[i] <= 0) error("weight must be > 0!");
        if(durations[i] <= 0) error("duaration must be > 0!");
        if(strength[i] <= 0) error("strength must be > 0!");
    }
